...
def get_file_name(path_to_file):
    ...
    file_name_with_format = os.path.basename(path_to_file)
    file_name = os.path.splitext(file_name_with_format)[0]

    return file_name

def get_seq_name(path_to_fasta):
    """Return name of the first sequence in fasta file"""

    with open (path_to_fasta, "r") as fasta_file:
        seq_name = fasta_file.readline()[1:].strip()
